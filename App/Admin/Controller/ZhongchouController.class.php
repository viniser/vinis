...
class ZhongchouController extends AdminController {
	...
	//解冻奖励
    public function jieById(){
		//if(I('post.t')!='jie'){$this->error('非法操作!');}
    	$id=I('get.id');
		$iid=I('get.iid');
		$name=I('get.name');
		$uid=I('get.uid');
		...
		if(!empty($id)){
    		$where[C("DB_PREFIX")."currency_dong.id"]=$id;
    	}
		if(!empty($iid)){
    		$where[C("DB_PREFIX")."currency_dong.iid"]=$iid;
			$ur.="url:";
    	}
    	if(!empty($uid)){
    		$where[C("DB_PREFIX")."currency_dong.uid"]=$uid;
    	}
    	if(!empty($name)){
    		...
    		$where[C("DB_PREFIX")."currency_dong.uid"]=array('in',$arr);
    	}
    	if(!empty($email)){
		...
		$count= M('Currency_dong')->where($where)->count();
		if($count==0){$this->error('没有数据可以操作!');}
		 
		 
    	$log=M('Currency_dong')->where($where)->select();
    	foreach ($log as $k=>$v){
    		if($v['status']==1){
    			continue;
    		}
    		$list[]=M('Currency_dong')->where("id={$v['id']}")->setField('deal',0);
    		$list[]=M('Currency_dong')->where("id={$v['id']}")->setField('add_time',time());
    		$list[]=M('Currency_user')->where("member_id={$v['uid']} and currency_id={$v['cid']}")->setInc('num',$v['deal']);
    		$list[]=M('Currency_user')->where("member_id={$v['uid']} and currency_id={$v['cid']}")->setDec('forzen_num',$v['deal']);
    		$list[]=M('Currency_dong')->where("id={$v['id']}")->setField('status',1);
    	}
    	if($list){
    		$this->success('操作成功');
    	}else{
	...
	//冻结奖励
    public function dongById(){
    	//if(I('get.t')!='dong'){$this->error('没有数据可以操作!');}
		$id=I('get.id');
		$iid=I('get.iid');
		$name=I('get.name');
		$uid=I('get.uid');
    	...
    	foreach ($log as $k=>$v){
    		...
    		$list[]=M('Currency_dong')->where("id={$v['id']}")->setField('deal',$v['num']);
    		$list[]=M('Currency_dong')->where("id={$v['id']}")->setField('add_time',time());
    		$list[]=M('Currency_user')->where("member_id={$v['uid']} and currency_id={$v['cid']}")->setInc('forzen_num',$v['num']);
    		$list[]=M('Currency_user')->where("member_id={$v['uid']} and currency_id={$v['cid']}")->setDec('num',$v['num']);
    		$list[]=M('Currency_dong')->where("id={$v['id']}")->setField('status',0);
    	}
    	if($list){
    		$this->success('操作成功');
    	}else{
    ...
    public function jiedong_start(){	
    	...
    	foreach ($log as $k=>$v){
    		...
    		$r=M(' ')->where("member_id={$v['uid']} and currency_id={$v['cid']}")->setInc('num',$v['num']*$v['remove_forzen_bili']);
    ...
    public function dongjie(){
		...
		$ur="";
    	if(!empty($iid)){
    		$where[C("DB_PREFIX")."currency_dong.iid"]=$iid; $ur['iid']=$iid;
    	}
    	if(!empty($uid)){
    		$where['yang_currency_dong.uid']=$uid;  $ur['uid']=$uid;
    	}
    	if(!empty($name)){
    		$uid=M('Member')->where("name like '%{$name}%'")->select();
			foreach($uid as $k=>$v){
    		...
    		$where[C("DB_PREFIX")."currency_dong.uid"]=array('in',$arr);   $ur['name']=$name;
    	}
    	if(!empty($email)){
    		$uid=M('Member')->where("email like '%{$email}%'")->select();
    		foreach($uid as $k=>$v){
    		...
    		$where['yang_currency_dong.uid']=array('in',$arr);    $ur['email']=$email;
    	}
    	$count      = M('Currency_dong')->count();// 查询满足要求的总记录数
    	$Page       = new \Think\Page($count,25);// 实例化分页类 传入总记录数和每页显示的记录数(25)
    	//给分页传参数
    	setPageParameter($Page, array('iid'=>$iid,'uid'=>$uid,'name'=>$name,'email'=>$email));
    	...
    	$show       = $Page->show();// 分页显示输出
    	$log=M('Currency_dong')
    	->field(C("DB_PREFIX")."Currency_dong.*,".C("DB_PREFIX")."member.name,".C("DB_PREFIX")."finance_type.name as title")
    	->join("left join ".C("DB_PREFIX")."member on ".C("DB_PREFIX")."member.member_id=".C("DB_PREFIX")."Currency_dong.uid")
    	->join("left join ".C("DB_PREFIX")."finance_type on ".C("DB_PREFIX")."finance_type.id=".C("DB_PREFIX")."Currency_dong.iid")
    	->order('id desc')
    	->limit($Page->firstRow.','.$Page->listRows)
    	->where($where)->select();
        foreach ($log as $key=>$vo) {
           $log[$key]['currency_name'] = $vo['cid']==0?"人民币":M('Currency')->where("currency_id='{$vo['cid']}'")->find()['currency_name'];
        }


        $this->assign('log',$log);
    	$this->assign('page',$show);
    	$issue=M('Finance_type')->field('id,name')->select(array('status'=>1));
    	$this->assign('issue',$issue);
		$this->assign('ur',$ur);
    	$this->assign('empty','暂无数据');
    	$this->display();
