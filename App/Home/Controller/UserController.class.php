...
class UserController extends HomeController {
    ...
    public function index(){
		...
		$status = M('Member')->field('status')->where($where)->find();
		if($status['status']!=1){
			 $this->redirect('ModifyMember/modify');   //ModifyMember/modify.html  User/index
            return;
		}
        ...
        foreach ($currency_user as $k=>$v){
            ...
            $allmoneys+=$allmoney;
			$currency_user[$k]['allmoney']=$allmoney;
        }
    ...
    public function updateMassage(){
        ...
        if(IS_POST){
			...
			$areas2 = M("Areas")->where(array('parent_id'=>$list['province']))->select();
    ...
    public function updatePassword(){
        ...
        if(IS_POST){
			...
			$data['status']=2;
                $data['info']='请输入6-20位密码#'.checkPwd($_POST['oldpwd']);
                $this->ajaxReturn($data);
    ...
    public function updatePwdTrade(){
       ...
       if(!$login){
        ...
        if(IS_POST){
			...
			//$this->success('申请成功,审核后会以系统通知通知您','http://baidu.com');
			...
			$data['status'] = 1;
			$m = $M_member->where(array('member_id'=>$member_id))->save(array('pwdtrade'=>$data['pwdtrade']));
...
//         $my_invit = M('Member')->field('email,status,reg_time')->where(array('pid'=>session('USER_KEY_ID')))->select();
        ...
        $my_invit = M('Member')->field('email,status,reg_time')->where(array('pid'=>session('USER_KEY_ID')))->limit($page->firstRow.','.$page->listRows)->order("member_id desc")->select();//时间降序排列，越接近当前时间越高
		...
		$Ncount=M('Member')->where("pid={$_SESSION['USER_KEY_ID']}")->count();
    ...
    public function sysMassage(){
        ...
        // 进行分页数据查询 注意limit方法的参数要使用Page类的属性   type_name
        $list = $M_member
            ->alias('a')
            ->field('a.message_id,a.message_all_id,c.title,c.type,c.add_time,a.status,b.name')
            ->where($where)
			->join(''.C('DB_PREFIX').'message_all as c on a.message_all_id = c.id')
            ->join(''.C('DB_PREFIX').'message_category as b on c.type = b.id')
            ->order(" a.status asc, c.add_time desc")
    ...
    public function showSysTem(){
        $message_id = I('id','','intval');
        $message_all_id = I('mid','','intval');
        $member_Id = session('USER_KEY_ID');
        //$where['member_id'] = $member_Id;
        ...
        $list = M('message')
            ->alias('a')
            ->field('c.title,a.status,c.type,c.add_time,c.content,b.name type_name')
            ->where($where)
			->join(C('DB_PREFIX').'message_all as c on a.message_all_id = c.id ')
            ->join(C('DB_PREFIX').'message_category as b on c.type = b.id')
            ->find();
        ...
        $right = M('Message')
            ->alias('a')
            ->field('a.message_id,a.message_all_id,c.title,c.type,c.add_time,b.name type_name')
            ->where($where)
			->join(C('DB_PREFIX').'message_all as c on a.message_all_id = c.id')
            ->join(C('DB_PREFIX').'message_category as b on c.type = b.id')
...
public function insert(){
        ...
        if(IS_POST){
			...
			$info['cardnum2']=I("account2"); 
            $info['bankname'] = I("bank");
			$info['branch']=I("bank_branch");  //支行名称
            $info['cardname'] = $this->auth['name'];
            $info['uid'] = session('USER_KEY_ID');
			
/*		    	$re = $bank->add($info); 
				$data['status']  = 0;
                $data['info'] = '请填写标签'.$info['address'];
                $this->ajaxReturn($data);*/
		...
		if($info['ctype']==1){		
			...
			if(empty($info['cardnum2'])||($info['cardnum2']!=$info['cardnum'])){
                $data['status']  = 4;
                $data['info'] = '请核对再次输入的银行卡号';
                $this->ajaxReturn($data);
            }
    ...
    public function draw(){
        ...
        $bank_info = $bank->field($field)->join(C('DB_PREFIX')."areas as b ON b.area_id =".C('DB_PREFIX')."bank.address")
            ->join(C('DB_PREFIX')."areas as a ON a.area_id = b.parent_id ")->order('id desc')->where($where)->select();
        //检测是否有10个地址
        ...
        //显示提现记录
        $draw_info = $withdraw
            ->field(C('DB_PREFIX')."withdraw.withdraw_id, ".C('DB_PREFIX')."bank.cardnum, ".C('DB_PREFIX')."bank.bankname, ".C('DB_PREFIX')."withdraw.all_money, ".C('DB_PREFIX')."withdraw.money, ".C('DB_PREFIX')."withdraw.add_time, ".C('DB_PREFIX')."withdraw.status")
